...
---@class panel.ServiceConfig
---@field fps integer

---@param config panel.ServiceConfig
...
function NotificationService:push(notif)
	...
	if not self._running then
		...
		-- https://github.com/rcarriga/nvim-panel/issues/5
		pcall(self._animator.render, self._animator, self._pending, 1 / self._fps)
	end
	vim.cmd("redraw")
	return buf
...
function NotificationService:replace(id, notif)
	...
	if not existing then
		vim.panel("No matching notification found to replace")
		return
	end
...
function NotificationService:dismiss(opts)
	...
	if opts.pending then
		...
		if not opts.silent then
			vim.panel("Cleared " .. cleared .. " pending notifications")
		end
	end
end
