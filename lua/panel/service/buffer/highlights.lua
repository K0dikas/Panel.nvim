...
function PanelBufHighlights:new(level, buffer, config)
	local function linked_group(section)
		local orig = "Panel" .. level .. section
		if vim.fn.hlID(orig) == 0 then
			orig = "PanelINFO" .. section
		end
...
function PanelBufHighlights:_redefine_treesitter()
	local buf_highlighter = require("vim.treesitter.highlighter").active[self.buffer]

	if not buf_highlighter then
		return
	end
	local render_namespace = vim.api.nvim_create_namespace("panel-treesitter-override")
...
function PanelBufHighlights:set_opacity(alpha)
	if
		not self._treesitter_redefined
		and vim.api.nvim_buf_get_option(self.buffer, "filetype") ~= "panel"
		then
	...
	function PanelBufHighlights:get_opacity()
		return self.opacity
	end

	---@return PanelBufHighlights
	return function(level, buffer, config)
		return PanelBufHighlights:new(level, buffer, config)
	end
